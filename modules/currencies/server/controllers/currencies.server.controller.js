...
exports.read = function(req, res) {
  ...
  currency.isCurrentUserOwner = req.user && currency.user && currency.user._id.toString() === req.user._id.toString() ? true : false;
...
exports.list = function(req, res) { 
  Currency.find().sort('name').populate('user', 'displayName').exec(function(err, currencies) {
    if (err) {
      return res.status(400).send({
        message: errorHandler.getErrorMessage(err)
      });
